...
(
	...
	input [MAX_BDIMBITS-1:0] i_go_blky,
	...
	input i_rddone_whichpage,
	
	// Interface to block compute engine
...
// Set after the very first block is written back.
// Processing of subsequent blocks by the comp thread depends on
// the current block being copied to the top block.

logic wb_donefirst;
bit wb_donefirst_set, wb_donefirst_clear;

always_ff @ (posedge clk_b or posedge reset) begin
	if (reset) wb_donefirst <= '0;
	else begin
		if (wb_donefirst_clear) wb_donefirst <= '0;
		else if (wb_donefirst_set) wb_donefirst <= '1;
	end
end
...
always_comb begin
	...
	case (cmp_state)
		...
		S_CMP_BPU_3: begin
			...
			// wb_donefirst is true when the previous current block has been copied to the top block.
			if (wb_donefirst && page_filled.cur) begin	
				o_comp_go = '1;
				o_comp_mode = MODE_2;
				cmp_nextstate = S_CMP_BPU_2;
			end
		...
		S_CMP_CPU_3: begin
			if (page_filled.cur && page_filled.left && wb_donefirst) begin
				o_comp_go = '1;
				o_comp_mode = MODE_4;
				cmp_nextstate = S_CMP_CPU_2;
			end
...
always_comb begin
	...
	o_wrreq_whichbufs_top = ~wb_donefirst; // wb_donefirst is 0 for first block, stays at 1 for all others
	cur_wb_page_flip = '0;
	cur_wb_page_reset = '0;
	blky_wb_inc = '0;
	page_free = '0;
	wb_donefirst_set = '0;
	wb_donefirst_clear = '0;
	
	case (wb_state)
		S_WB_IDLE: begin
			wb_donefirst_clear = '1;
			cur_wb_page_reset = '1;
		...
		S_WB_2: begin
			if (i_wrreq_ack) wb_nextstate = S_WB_3;
		end
